...
listeJoueurs <- rio::import("1_joueurs/licencies 2020.txt") %>% 
  ...
  arrange(desc(`Type de licence`),desc(`Pts (début saison)`),`Prénom`)

listeJoueurs%>% 
  formaterTableau %>% 
    formatStyle(1:9,valueColumns = 9, 
                color = styleInterval(c(900,1000,1100), c('darkBlue','white','greenYellow',"yellow")),
                backgroundColor = styleInterval(c(600,700,800,900,1000,1100), c('lightCyan', 'PowderBlue','LightSteelBlue','DodgerBlue','RoyalBlue','MediumBlue',"DarkBlue"))
    )
# }
```
...
```{r EvolJoueurs_Graph, echo = FALSE, fig.align='center'}
library(billboarder)
library(dplyr)
evolutionJoueurs <- read.csv2("1_joueurs/evolutionJoueurs_Tests.csv")
billboarder() %>% 
  bb_linechart(
...
<!-- <details> -->
<!-- <summary markdown="span">**Cliquez sur la flèche pour sélectionner le tableau de données**</summary> -->


```{r EvolJoueurs_Donnees, echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, paged.print=FALSE}
library(DT)
library(dplyr)

EvolJoueurs_Donnees <- read.csv2("1_joueurs/evolutionJoueurs_Tests.csv") %>% 
  arrange(desc(ANNEE))
EvolJoueurs_Donnees %>% 
  formaterTableau %>% 
  formatStyle(2:length(EvolJoueurs_Donnees), 
                color = styleInterval(c(900,1000,1100), c('darkBlue','white','greenYellow',"yellow")),
                backgroundColor = styleInterval(c(600,700,800,900,1000,1100), c('lightCyan', 'PowderBlue','LightSteelBlue','DodgerBlue','RoyalBlue','MediumBlue',"DarkBlue"))
    )
